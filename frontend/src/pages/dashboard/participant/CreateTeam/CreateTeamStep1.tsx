...
const step1Schema = z.object({
  ...
  captain_phone: z.string().min(1, "Phone number is required"),
  captain_contact: z.string().min(1, "Telegram or Discord username is required"),
});
          ...
          <div className={styles.registrationContent}>
            <div className={styles.registrationHeader}>
              ...
              <p className={styles.registrationSubtitle}>
                Provide the foundational details for your team. You will add members and select a tournament in the next steps.
              </p>
            </div>
            ...
            <form className={styles.registrationForm} onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className={styles.formRow}>
                ...
                <div className={`${styles.field} ${styles.fieldHalf}`}>
                  ...
                  />
                  {errors.organization && (
                    <span className={styles.fieldError}>{errors.organization.message}</span>
                  )}
                </div>
              </div>
              ...
              <div className={styles.formRow}>
                <div className={`${styles.field} ${styles.fieldHalf}`}>
                  ...
                  <input
                    ...
                    className={`${styles.input} ${errors.captain_phone ? styles.inputError : ""}`}
                    {...register("captain_phone")}
                  />
                  {errors.captain_phone && (
                    <span className={styles.fieldError}>{errors.captain_phone.message}</span>
                  )}
                </div>

                <div className={`${styles.field} ${styles.fieldHalf}`}>
                  <label className={styles.label}>Telegram / Discord *</label>
                  <input
                    type="text"
                    placeholder="@username"
                    className={`${styles.input} ${errors.captain_contact ? styles.inputError : ""}`}
                    {...register("captain_contact")}
                  />
                  {errors.captain_contact && (
                    <span className={styles.fieldError}>{errors.captain_contact.message}</span>
                  )}
                </div>
              </div>
