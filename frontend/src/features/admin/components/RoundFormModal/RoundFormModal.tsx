...
import { z } from "zod";
import { useCreateRound, useUpdateRound } from "@/features/admin/hooks/useRounds";
import { useTournaments } from "@/features/Tournaments/hooks/useTournaments";
import type { Task } from "@/services/api";
import styles from "./RoundFormModal.module.css";
...
export default function RoundFormModal({ round, defaultTournamentId = 0, onClose }: Props) {
  const isEdit = !!round;
  ...
  useEffect(() => {
    if (round) {
      reset({
        ...
        specifications: round.specifications,
  ...
  }, [round, defaultTournamentId, reset, setValue]);

  const onSubmit = (data: RoundForm) => {
    const payload = {
      ...data,
      status: (round?.status || "Draft") as Task["status"],
    };

    if (isEdit && round) {
      updateMutation.mutate(
        { id: round.id, data: payload },
        {
          onSuccess: () => onClose(),
          onError: (err) =>
            setError("root", {
    ...
    <div className={styles.overlay}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2 className={styles.title}>{isEdit ? "Edit Round" : "Create Round"}</h2>
        ...
        <form className={styles.form} onSubmit={handleSubmit(onSubmit)} autoComplete="off">
          ...
          <div className={styles.actions}>
            ...
            <button type="submit" className={styles.btnPrimary} disabled={isPending}>
              {isPending ? "Saving…" : isEdit ? "Save Changes" : "Create Round"}
            </button>
          </div>
        </form>
      </div>
